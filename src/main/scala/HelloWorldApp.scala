...
import model._
import controller._
import repository.doobie.{DoobieAuthRepositoryInterpreter, DoobieTourRepositoryInterpreter, DoobieUserRepositoryInterpreter}
import validation.{TourValidationInterpreter, UserValidationInterpreter}
...
import service.{TourService, UserService}
...
object HelloWorldApp extends IOApp {
  def createServer[F[_]: ContextShift: ConcurrentEffect: Timer]: Resource[F, H4Server[F]] =
    for {
      ...
      tourRepo = DoobieTourRepositoryInterpreter[F](xa)
      userValidation = UserValidationInterpreter[F](userRepo)
      tourValidation = TourValidationInterpreter[F](tourRepo)
      userService = UserService[F](userRepo, userValidation)
      tourService = TourService[F](tourRepo, tourValidation)
      ...
      httpApp = Router(
        ...
        "/tours" -> TourController
          .endpoints[F, HMACSHA256](tourService, routeAuth)
      ).orNotFound
